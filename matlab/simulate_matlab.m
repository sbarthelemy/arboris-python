function simulate_matlab(nb_bd, T0, gravity, dt, tend, sim_name)
    ...
    r(1).T = T0(:);
    r(1).controller = arb_pdcontroller(r(1));
    r(1).model.GravityAcceleration = gravity(:);
    
    convert_robot(r, 'robot_generic.py')
    %% Simulation
    ...
    M = arrayfun(@(x)( x.controller.M),rs(1,1:end),'UniformOutput',0);
    N = arrayfun(@(x)( x.controller.N),rs(1,1:end),'UniformOutput',0);
